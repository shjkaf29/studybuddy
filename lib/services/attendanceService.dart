...
class AttendanceService extends ChangeNotifier {
  ...
  Future<Map<String, List<Attendance>>> getAttendanceBySubject() async {
    await _loadAttendance();
    final Map<String, List<Attendance>> bySubject = {};
    
    for (var record in _attendance) {
      if (!bySubject.containsKey(record.subject)) {
        bySubject[record.subject] = [];
      }
      bySubject[record.subject]!.add(record);
    }
  ...
  Future<void> _loadAttendance() async {
    try {
      ...
      if (data != null) {
        ...
        _attendance.clear();
        _attendance.addAll(
          decoded.map((item) => Attendance.fromMap(item)).toList(),
        );
      }
    } catch (e) {
